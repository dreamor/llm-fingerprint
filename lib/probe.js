...
export async function probe(opts) {
  ...
  const runOne = async (unit) => {
    ...
    try {
      const raw = await withRetry(
        ...
        {
          retries,
          onRetry: (attempt, delay, err) => emit('retry', { unit, attempt, delay, error: err.message }),
        },
      );
  ...
  if (!adaptive) {
    ...
    await runAdaptive({
      workItems, cellKeys, cellUnits, runOne,
      concurrency, model, temperature,
...
function buildCell(task_id, lang, temperature, rawAnswers) {
  ...
  for (const k of Object.keys(dist)) {
    dist[k] = nValid > 0 ? Math.round((dist[k] / nValid) * 10000) / 10000 : 0;
  }
  const entropy = computeEntropy(dist);
  const mode = Object.entries(dist).sort((a, b) => b[1] - a[1])[0];
  return {
    task_id, lang, temperature,
    n_valid: nValid,
    n_off_format: rawAnswers.length - nValid,
    validity_rate: rawAnswers.length > 0 ? Math.round((nValid / rawAnswers.length) * 10000) / 10000 : 0,
    dist,
 ...
 */
async function runAdaptive({ workItems, cellKeys, cellUnits, runOne, concurrency, model, temperature, matcher, db, stopSignal, emit }) {
  ...
  for (const rep of sortedReps) {
    ...
    emit('adaptive_check', { reps: repsSoFar, top: top?.model ?? null, jsd: top?.mean_jsd ?? null });
