...
export async function run({ db, positional, flags }) {
  ...
  if (!endpoint || !model) {
    warn('usage: fp probe <endpoint> [<api-key>|-] <model> [--reps N|auto] [--langs en,zh] [--api openai|anthropic] [--concurrency 4] [--adaptive]');
    process.exit(1);
  }
  ...
  const eerTarget = flags.eer ? parseFloat(flags.eer) : 0.10;
  ...
  console.log(`  reps:        ${reps}${repsRaw === 'auto' ? ` (auto — budget EER ≤ ${(eerTarget * 100).toFixed(0)}%)` : ''}`);
  ...
  const ts = Date.now();
  const outDir = join(PKG_ROOT, 'data', `probe-${ts}`);
