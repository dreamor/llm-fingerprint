...
export async function run({ db, positional, flags }) {
  ...
  const { key: apiKey, source } = resolveApiKey({
    positional: apiKeyPositional, flags, apiType, warn,
  });
  if (source !== 'positional') warn(`api key source: ${source}`);

  const reps = resolveReps(flags.reps || '16', 0.10);
  ...
  const bar = createProgress({ total: reps * 15 * languages.length, label: `verify ${claimedModel}` });
  const result = await probe({
    endpoint, apiKey, model: claimedModel,
    apiType, temperature: 1, reps, languages,
    concurrency, extraBody,
    onEvent: ev => {
  ...
  const outDir = join(PKG_ROOT, 'data', `verify-${Date.now()}`);
  ...
  if (!claimedEntry) {
    console.log(`  ❌ VERIFICATION FAILED — "${claimedModel}" did not clear the shared-cells threshold.`);
    console.log(`═══════════════════════════════════════\n`);
    return;
  }
