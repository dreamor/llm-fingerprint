...
const TIERS = [
  ...
  { key: 'high',      maxJsd: 0.10, label: 'high confidence — same or very close variant' },
  { key: 'moderate',  maxJsd: 0.20, label: 'moderate confidence — same family' },
  { key: 'low',       maxJsd: 0.30, label: 'low confidence — loose resemblance' },
...
export function verificationVerdict(jsd, claimed, bestAlternative) {
  ...
  if (tier.key === 'moderate') {
    return {
      ...
      detail: 'Behavior is in the same family but the fingerprint is not clean. Possible: model update, quantization, or provider routing variance.',
    };
  }
  if (tier.key === 'low') {
    return {
      ...
      detail: bestAlternative && bestAlternative !== claimed
