...
export function resolveApiKey({ positional, flags, apiType, warn }) {
  if (positional && positional !== '-') {
    ...
    warn?.('api key passed positionally is visible in `ps` output; prefer --api-key-env or --api-key-file');
  ...
  throw new Error(`no api key — pass positionally, use --api-key-env NAME, --api-key-file PATH, or set one of: ${envFallbacks.join(', ')}`);
}
