...
export function validateDistribution(record) {
  ...
  if (typeof record.model !== 'string' || record.model.length === 0) errors.push('model must be a non-empty string');
  if (typeof record.task_id !== 'string' || record.task_id.length === 0) errors.push('task_id must be a non-empty string');
  if (typeof record.lang !== 'string' || record.lang.length === 0) errors.push('lang must be a non-empty string');
  if (typeof record.temperature !== 'number' || !Number.isFinite(record.temperature)) errors.push('temperature must be a finite number');
  if (record.dist !== undefined && record.dist !== null) {
    if (typeof record.dist !== 'object' || Array.isArray(record.dist)) {
      ...
      for (const [k, v] of Object.entries(record.dist)) {
        if (typeof k !== 'string' || k.length === 0) errors.push(`dist key must be a non-empty string, got: ${JSON.stringify(k)}`);
...
export function validateDistributions(records) {
  if (!Array.isArray(records)) {
    return { valid: [], invalid: [{ record: records, errors: ['input is not an array'], index: -1 }] };
  }
  const valid = [];
  const invalid = [];
