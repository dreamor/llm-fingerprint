...
export class FingerprintDB {
  ...
  save(records) {
    const targetPath = process.env.LLM_FINGERPRINT_HOME
      ? this.path
      : (this.path === bundledReferencePath() ? userReferencePath() : this.path);
    ...
    if (existsSync(targetPath)) {
      ...
      // First-time write to user dir — seed from bundled so nothing is lost
    ...
    console.log(`[db] saved ${merged.length} distribution cells (${this._index.size} models) to ${targetPath}`);
  }
  ...
  getModel(model) { return this._index.get(model); }

  /** List all known models */
  listModels() { return [...this._index.keys()].sort(); }
...
export function inferFamily(modelSlug) {
  ...
  const map = {
    ...
    'qwen': 'qwen', 'cohere': 'command', 'z-ai': 'glm', 'x-ai': 'grok',
    'amazon': 'nova', 'nvidia': 'nemotron', 'moonshotai': 'kimi',
    'tencent': 'hunyuan', 'bytedance-seed': 'seed', 'minimax': 'minimax',
    'ibm-granite': 'granite', 'microsoft': 'microsoft', 'nousresearch': 'nous',
    'baidu': 'ernie', 'inflection': 'inflection', 'arcee-ai': 'arcee',
    'nex-agi': 'nex', 'writer': 'writer', 'xiaomi': 'xiaomi',
    'upstage': 'upstage', 'inception': 'inception', 'inclusionai': 'ling',
    'liquid': 'liquid', 'deepcogito': 'cogito', 'rekaai': 'reka',
    'perceptron': 'perceptron', 'ai21': 'ai21', 'aion-labs': 'aion',
  };
