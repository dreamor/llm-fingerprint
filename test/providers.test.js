...
describe('openai provider', () => {
  it('builds a completions URL from a base', () => {
    assert.equal(openai.chatCompletionsUrl('https://api.openai.com/v1'), 'https://api.openai.com/v1/chat/completions');
  });

  it('does NOT send OpenRouter reasoning field by default', () => {
    const req = openai.buildRequest({ apiKey: 'k', model: 'm', prompt: 'p', temperature: 1, maxTokens: 16 });
  ...
  it('merges extraBody when provided (for OpenRouter opt-in)', () => {
    const req = openai.buildRequest({
      apiKey: 'k', model: 'm', prompt: 'p', temperature: 1, maxTokens: 16,
...
describe('anthropic provider', () => {
  it('adds /v1/messages to bare host', () => {
    assert.equal(anthropic.messagesUrl('https://api.anthropic.com'), 'https://api.anthropic.com/v1/messages');
  });

  it('appends /messages when /v1 present', () => {
    assert.equal(anthropic.messagesUrl('https://api.anthropic.com/v1'), 'https://api.anthropic.com/v1/messages');
  });
  ...
  it('extracts concatenated text blocks', () => {
    const text = anthropic.extractText({ content: [
      { type: 'text', text: 'foo' },
      { type: 'text', text: 'bar' },
